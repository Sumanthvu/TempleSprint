...
export default function ScoreScreen({
  ...
  const { writeContract, data: txHash, isPending: isClaimSubmitting } = useWriteContract();
  const claimHash = txHash as `0x${string}` | undefined;
  const { isLoading: isClaimMining, isSuccess: isClaimMined } = useWaitForTransactionReceipt({ hash: claimHash });
  ...
  useEffect(() => {
    if (!isClaimMined || !claimHash || !address || syncedHash === claimHash || !context.fid) return;
  ...
  const connectWallet = useCallback(() => {
    const preferredConnector = connectors.find((connector) => connector.id === "metaMask") ?? connectors[0];
  ...
  const claimReward = useCallback(async () => {
    ...
    try {
      ...
      if (intentData?.alreadyClaimed) {
        setRewardClaimed(true);
        setWalletOnRecord(address ?? null);
        return;
      }
  ...
  const getGrade = (s: number) => {
    if (s >= 2000) return { label: "ELITE", color: "#ffffff", emoji: "⚡" };
    if (s >= 1000) return { label: "CHAMPION", color: "#a1a1aa", emoji: "🏆" };
    if (s >= 500) return { label: "WARRIOR", color: "#71717a", emoji: "⚔️" };
          ...
          {targetTier === 0 && (
            ...
            >
              ...
              <strong style={{ color: "#D97706" }}>
                {Math.max(0, (nextMilestone ?? REWARD_THRESHOLDS.bronze) - score)}
              </strong>{" "}
